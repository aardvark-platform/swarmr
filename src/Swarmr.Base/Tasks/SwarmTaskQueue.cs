...
public class SwarmTaskQueue
{
    ...
    public Task Enqueue(ISwarmTask task)
    {
        ...
        AnsiConsole.MarkupLine($"[aqua][[SwarmTaskQueue]][[ENQUEUE]]{task.GetType().FullName.EscapeMarkup()}[/]");
    ...
    public Task<ISwarmTask?> TryDequeue()
    {
        ...
        if (result != null)
        {
            AnsiConsole.MarkupLine($"[teal][[SwarmTaskQueue]][[DEQUEUE]]{result.GetType().FullName.EscapeMarkup()}[/]");
        }
